...
| **M4** Sesión (constantes/accesores) | ✅ Hecho (acotado) | _(este commit)_ | Nuevo `Helpers/SesionHelper.cs` (constantes `ClaveUsuarioId/ClaveRol/ClaveNombre` + accesores `ObtenerUsuarioId/ObtenerNombre/ObtenerRolNormalizado` con semántica idéntica al código original). Adoptado en los 4 bloques de lectura de sesión idénticos de LiquidacionesPendientes (dedup behavior-identical). Build + 186 tests. |
| **M4b** Consolidar RolesHelper→SecurityHelper | ⛔ Diferido | — | **Riesgo/ambiguo**: cambia autorización real, toca `Login` (autenticación) y patrones de sesión heterogéneos (`.Trim().ToUpper()` vs `.ToUpperInvariant()`, accesos directos sin null-check). No es swap mecánico; requiere decisión explícita y prueba en runtime. Adopción masiva de `SesionHelper` en los demás call-sites también pendiente (incremental). |
